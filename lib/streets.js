...
const STREET_LOOKUPS /*: Array<[string,Street]> */ = (aliases => {
  ...
  const aliasToStreetLookups = [];
  for (const street of streets) {
    aliasToStreetLookups.push([street, street]);
    for (const alias of (aliases.get(street) || [])) {
      aliasToStreetLookups.push([alias, street]);
    }
  }
