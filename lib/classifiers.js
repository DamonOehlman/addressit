...
function joinSectionSeparatedRegions(input) {
  ...
  const head = input.slice(0, firstRegionIndex-1);
  const tail = input.slice(firstRegionIndex, input.length - firstRegionIndex);

  let collectedRegionData = [];
  const compressedRegions = tail.reduce((memo, part) => {
    if (PartClassifier.matches(part, 'SECTION_SEPARATOR')) {
      const newPart = {
        partType: 'REGION',
        value: collectedRegionData.join('')
      };

      collectedRegionData = [];
      return memo.concat(newPart);
    } else {
