...
function tagRegions(input) {
  const noUnknowns = input.filter(part => part.partType !== 'UNKNOWN');
  const regions = input.reduce((memo, part) => {
    if (part.partType === 'UNKNOWN') {
      return [].concat(PartBuilder.create('REGION', part.value));
    }

    return part;
  }, []);

  return noUnknowns.concat(regions);
}
