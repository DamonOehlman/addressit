...
function checkForUnitPrefix(input, locale) {
  ...
  return input.reduce((memo, part, index) => {
    ...
    if (match && match[1]) {
      ...
      const newParts = [PartBuilder.create('UNIT', match[1])];

      // if we have a second capture group content we have something in the format
      // like 7N234 which means we need to separate this part into a unit and street
      // number part, and concat both
      if (match[2]) {
        newParts.push(PartBuilder.create('NUMBER', match[2]));
      }

      return memo.concat(newParts);
